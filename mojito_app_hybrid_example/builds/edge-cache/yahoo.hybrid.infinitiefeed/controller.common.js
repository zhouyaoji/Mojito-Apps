    ...
    */
    ...
    Y.mojito.controllers[NAME] = {
        ...
        feed: function (ac) {
            ...
            ac.model.load('user').getConfig('user_id', function (error, feedIds) {

                ac.model.load('infinitiefeed').getFeed(feedIds, offset, limit, function (items) {
        ...
        updateTopic: function (ac) {
            ...
            ac.model.load('user').toggleFeed('user_id', topic, function (error) {
                // This could be better by only removing feedIds that were dropped.
                ac.model.load('infinitiefeed').emptyCache();
                ac.done("done");
            });
        }
...
}, '0.0.1', {requires: ['mojito', 'mojito_mojit_addon_shared_model', 'yahoo_hybrid_app_usermodel']});
